...
//#define ARDUINOLOG_SERIAL
#define ARDUINOLOG_SERIAL1
...
public:
...
#if defined(ARDUINOLOG_SERIAL)
     Serial.begin(115200);
     begin (_level, &Serial);
#elif defined(ARDUINOLOG_SERIAL1)
     //Serial1.begin(115200);
     Serial1.begin(9600);
     begin (_level, &Serial1);
#elif defined(ARDUINOLOG_SERIAL2)
     Serial2.begin(9600);
     begin (_level, &Serial2);
#elif defined(ARDUINOLOG_SERIAL3)
     Serial3.begin(115200);
     begin (_level, &Serial3);
#elif defined(ARDUINOLOG_SERIAL4)
     Serial4.begin(9600);
     begin (_level, &Serial4);
  ...
  char *ftoa(double f, int precision);
	...
	/**
	 * Output a plot message. Output message contains
	 * Fatal error messages are printed out at
	 * loglevels >= LOG_LEVEL_PLOT
	 * 
	 * \param msg format string to output
	 * \param ... any number of variables
	 * \return void
	 */
	template <class T, typename... Args> void plot(T msg, Args... args)
	{
#ifndef DISABLE_LOGGING
		printLevel(LOG_LEVEL_PLOT, msg, args...);
#endif
	}
...
private:
  void print1(PGM_P format,...);
	...
	{
#ifndef DISABLE_LOGGING
    if ( _level != LOG_LEVEL_PLOT && level > _level)
       return;
       
    if ( _level != LOG_LEVEL_PLOT && level == LOG_LEVEL_PLOT )
       return;
