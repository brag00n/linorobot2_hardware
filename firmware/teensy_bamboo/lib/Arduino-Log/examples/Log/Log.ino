...
void setup() {
    ...
    Log.trace(F(CR "******************************************" CR));                     // Info string with Newline
    Log.trace(  "***          Logging example                " CR);                       // Info string in flash memory
    Log.trace(F("******************* ")); Log.trace("*********************** " CR);      // two info strings without newline
}
...
void loop() {
    ...
    Log.trace   (  "Log as Info with integer values : %d, %d" CR                  , intValue1,  intValue2);
    Log.trace   (F("Log as Info with hex values     : %x, %X" CR                 ), intValue1,  intValue1);
    Log.trace   (  "Log as Info with hex values     : %x, %X" CR                  , intValue2,  intValue2);
    Log.trace   (F("Log as Info with binary values  : %b, %B" CR                 ), intValue1,  intValue1);
    Log.trace   (  "Log as Info with binary values  : %b, %B" CR                  , intValue2,  intValue2);
    Log.trace   (F("Log as Info with long values    : %l, %l" CR                 ), longValue1, longValue2);
    Log.trace   (  "Log as Info with bool values    : %t, %T" CR                  , boolValue1, boolValue2);
    Log.trace   (F("Log as Info with string value   : %s" CR                     ), charArray);
    Log.trace   (  "Log as Info with Flash string value   : %S" CR                , flashCharArray1);
    Log.trace   (  "Log as Info with Flash string value   : %S" CR                , flashCharArray2);
    Log.trace   (  "Log as Info with string value   : %s" CR                      , stringValue1.c_str());
    Log.trace   (F("Log as Info with float value   : %F" CR                      ), floatValue);
    Log.trace   (  "Log as Info with float value   : %F" CR                       , floatValue);
    Log.trace   (F("Log as Info with double value   : %D" CR                     ), doubleValue);
    Log.trace   (  "Log as Info with double value   : %D" CR                      , doubleValue);
    Log.trace   (F("Log as Debug with mixed values  : %d, %d, %l, %l, %t, %T" CR ), intValue1 , intValue2,
