...
bool Raspi_PWM_Servo_Driver::setPWM(uint8_t channel, uint16_t on, uint16_t off){
    ...
    //if (!I2Cdev::writeByte(_address, LED0_ON_H +4*channel,  on >> 8))     return false;
    //if (!I2Cdev::writeByte(_address, LED0_OFF_L+4*channel,  off & 0xFF)) return false;
    //if (!I2Cdev::writeByte(_address, LED0_OFF_H+4*channel,  off >> 8))   return false;
