...
void moveBase()
{
  ...
  if(true ||twist_msg.linear.x != 0 || twist_msg.linear.y != 0 || twist_msg.angular.z != 0) {
    ...
    long pos_motor3=motor3_encoder.read();
    long pos_motor4=motor4_encoder.read();
    pos_motor1_previous=pos_motor1;
    pos_motor2_previous=pos_motor2;
    pos_motor3_previous=pos_motor3;
    pos_motor4_previous=pos_motor4;

    char buffer[200];
    if (joint_rpm[0]!=0 || joint_rpm[1]!=0 || joint_rpm[2]!=0 || joint_rpm[3]!=0 ||
        pos_motor1!=pos_motor1_previous ||
        pos_motor2!=pos_motor2_previous ||
        pos_motor3!=pos_motor3_previous ||
        pos_motor4!=pos_motor4_previous 
    ){
       sprintf(buffer, "[POS1:%6d, RPM1:%6d, REQ_RPM1:%6d, PWM1_PID:%6d] [POS2:%6d, RPM2:%6d, REQ_RPM2:%6d, PWM2_PID:%6d]",
          ...
          pos_motor3,(long) joint_rpm[2],(long) round(request_rpm.motor3),(long) round(smooth_pwm[2]),
          pos_motor4,(long) joint_rpm[3],(long) round(request_rpm.motor4),(long) round(smooth_pwm[3]));
       log(1,buffer);
    }
    ...
    motor3_controller.spin(smooth_pwm[2]);
    motor4_controller.spin(smooth_pwm[3]);
