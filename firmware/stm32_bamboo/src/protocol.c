...
void Upper_Data_Parse(uint8_t *data_buf, uint8_t num)
{
	...
	/* Saut vers le bootloader ROM : flash logiciel sans BOOT0+RESET.
	 * Protege par SAVE_VERIFY pour eviter tout declenchement accidentel. */
	case FUNC_ENTER_BOOTLOADER:
	{
		uint8_t verify = *(data_buf + 4);
		if (verify == SAVE_VERIFY)
		{
			DEBUG("enter bootloader\n");
			Bsp_Jump_To_Bootloader();
		}
		break;
	}
