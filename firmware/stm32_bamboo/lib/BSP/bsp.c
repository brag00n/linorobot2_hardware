...
/* --- Entree en bootloader IAP resident (flash sans BOOT0+RESET) ---
 * Le bootloader (16 KB en 0x08000000, projet bootloader/) tourne au reset AVANT
 * cette application. On lui signale de rester en mode reception via un flag place
 * en RAM haute (hors linkers, survit au NVIC_SystemReset), puis on redemarre.
 * Contrat partage : lib/IAP/iap_protocol.h. */
void Bsp_Jump_To_Bootloader(void)
{
	IAP_FLAG = IAP_FLAG_UPDATE;   // le bootloader lira ce flag au demarrage
	__set_FAULTMASK(1);
	NVIC_SystemReset();
}
