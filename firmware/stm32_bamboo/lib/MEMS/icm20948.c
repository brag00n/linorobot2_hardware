...
void process_sensor_output()
{
    ...
    if (quat6_data_was_set == 1)
    {
        if (hal.report & PRINT_GRV)
        {
            ...
            values[0] = -asin(2 * (q02 - q13));                          // Roll ������
            values[1] = atan2(2 * (q03 + q12), (1 - 2 * (q22 + q33)));   // Pitch ������
            values[2] = atan2(2 * (q01 + q23), (1 - 2 * (q11 + q22)));   // Yaw ƫ����
...
int ICM_20948_Init(void)
{
    ...
    tx = 0x80;
    result |= inv_serial_interface_write_hook(0x06, 1, &tx); //��λ
    delay_ms(100);
    // App_Delay_ms(100);
    tx = 0x00;
    result |= inv_serial_interface_write_hook(0x7f, 1, &tx);
...
{
    return g_icm_data.orientation[0];         // ���ػ���
    // return g_icm_data.orientation[0]*RtA;  // ���ؽǶ�
}

// ��ȡ��ǰ������
float ICM20948_Get_Pitch_Now(void)
{
    return g_icm_data.orientation[1];         // ���ػ���
    // return g_icm_data.orientation[1]*RtA;  // ���ؽǶ�
}

// ��ȡ��ǰƫ����
float ICM20948_Get_Yaw_Now(void)
{
    return g_icm_data.orientation[2];         // ���ػ���
    // return g_icm_data.orientation[2]*RtA;  // ���ؽǶ�
}
...
void ICM20948_Send_Raw_Data(void)
{
	...
	data_buffer[2] = LEN-2; // ����
	data_buffer[3] = FUNC_REPORT_ICM_RAW; // ����λ
...
void ICM20948_Send_Attitude_Data(void)
{
	...
	data_buffer[0] = PTO_HEAD;
	data_buffer[1] = PTO_DEVICE_ID-1;
	data_buffer[2] = LENS-2; // ����
	data_buffer[3] = FUNC_REPORT_IMU_ATT;    // ����λ
	Proto_Put_U32_LE(&data_buffer[4], Proto_Now_Ms()); // timestamp horloge interne (ms, u32 LE)
