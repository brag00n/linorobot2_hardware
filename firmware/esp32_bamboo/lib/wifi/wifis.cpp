...
void initWifis(void)
{
    ...
    if (isSyslog) syslog(LOG_DEBUG, "%s ssid %s rssi %d ip %s", __FUNCTION__, WiFi.SSID(), WiFi.RSSI(),
...
#ifdef WIFI_MONITOR
    EXECUTE_EVERY_N_MS(WIFI_MONITOR * 60 * 1000, if (isSyslog) syslog(LOG_DEBUG, "%s ssid %s rssi %d", \
