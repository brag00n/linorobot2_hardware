...
unsigned char QMI8658::get_id(void)
{
	...
	while(iCount<2)
	{
		...
		while((qmi8658_chip_id != 0x05)&&(retry++ < 5))
		{
			...
			if (isSyslog) syslog(LOG_DEBUG, ("   Qmi8658Register_WhoAmI = 0x" + String(qmi8658_chip_id) + "\n").c_str());
		}
...
{
	if (isSyslog) syslog(LOG_DEBUG, ("   qmi8658_on_demand_cali start\n"));
	...
	delay(100);	// delay
	if (isSyslog) syslog(LOG_DEBUG, "   qmi8658_on_demand_cali done\n");
