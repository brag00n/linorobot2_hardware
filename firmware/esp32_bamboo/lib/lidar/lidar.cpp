...
{
  if (isSyslog) syslog(LOG_DEBUG, "%s err %d", __FUNCTION__, err);
}
...
void rx_callback(void)
{
  ...
  // if (isSyslog) syslog(LOG_DEBUG, "%s recv %d", __FUNCTION__, cc);
  // send larger UDP packet
  if ((len += cc) && (len > 128)) {
      udp.beginPacket(LIDAR_SERVER, LIDAR_PORT);
      udp.write(buf, len);
      udp.endPacket();
      // if (isSyslog) syslog(LOG_DEBUG, "%s send %d", __FUNCTION__, len);
      len = 0;
  }
}
