r"""Telemetry - Journal structure du comportement du tooling robot_control.

L'orchestrateur (robot_control.RobotMain) detient COM4 ET la camera : aucun autre
process ne peut les ouvrir. Pour permettre une ANALYSE externe (ex : serveur MCP),
il ecrit son comportement sur disque, et l'observateur se contente de lire :
...
Classe volontairement conservee a l'identique de l'ancienne version (API snake_case
log/log_rx/snapshot/close) : c'est un utilitaire pur, non un organe RobotXxx.
"""
import json
import os
import threading
